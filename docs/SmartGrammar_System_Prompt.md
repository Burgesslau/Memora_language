...
> **版本**：v0.2.3 | **最后更新**：2026-06-08 | **当前阶段**：Phase 1 ✅ → Phase 2 准备中  
...
│       └── logging_utils.py
├── docs/
│   ├── SmartGrammar_System_Prompt.md    # 本文档（Agent 系统提示）
...
- **已完成**：
  ...
  - P1/P2 Bug 修复：瓶颈诊断 Stale Closure、失败计数重置、Sidebar 双高亮
  - 前后端图谱节点对齐（含 `passive_voice`）
  - 后端 pytest 单元测试（DualTrackParser / LexiconEngine / KnowledgeGraph）
...
| BUG-002 | P1 | 瓶颈诊断 `failureCounts` Stale Closure | ✅ 已修复 |
| BUG-003 | P2 | 答题通过后失败计数不重置 | ✅ 已修复 |
...
| DRIFT-004 | P2 | 前端 `passive_voice` 节点在后端内存图缺失 | ✅ 已同步 |
| WARN-001 | P2 | InlineHighlight `<p>` 嵌套 hydration 警告 | ✅ 已修复 |
| WARN-003 | P2 | Sidebar `/dashboard` 与 `/dashboard/grammar` 导航同时高亮 | ✅ 已修复 |

### 7.2 已知技术债（待后续迭代）

| ID | 说明 | 计划阶段 |
|----|------|----------|
| WARN-007 | `lexicon.py` 使用 Pydantic v1 弃用 API `entry.dict()` | Phase 2 |
| WARN-008 | Exam 页诊断语法点与 API 返回 error_tags 可能不一致 | Phase 2 |
